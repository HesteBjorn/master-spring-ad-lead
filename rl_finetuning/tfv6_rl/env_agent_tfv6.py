...
class EnvAgentTFv6(BaseAgent, autonomous_agent.AutonomousAgent):
    ...
    def set_global_plan(self, global_plan_world_coord):
        ...
        global_plan_gps = route_manipulation.location_route_to_gps(
            global_plan_world_coord, lat_ref, lon_ref
        )
        ds_ids = route_manipulation.downsample_route(global_plan_world_coord, 200)
        self._global_plan_world_coord = [
            (global_plan_world_coord[i][0], global_plan_world_coord[i][1])
            for i in ds_ids
        ]
        self._global_plan = [global_plan_gps[i] for i in ds_ids]
        ...
        self.socket.send_multipart(
            (
                *packed_obs,
                ...
                np.array(info.get("reward_speed_penalty", 1.0), dtype=np.float32),
                np.array(info.get("reward_ttc_penalty", 0.0), dtype=np.float32),
                np.array(info.get("reward_comfort_penalty", 1.0), dtype=np.float32),
                np.array(info.get("reward_lane_centering", 1.0), dtype=np.float32),
                np.array(info.get("reward_outside_lanes", 0.0), dtype=np.float32),
